import { business, communityExamples, communityThemes, impactFigures, menuHighlights, photoSlots } from './content.mjs';
...
export const pages = [
  ...
  },
  {
    ...
    body: `
      <section class="page-hero page-hero--catering"><div class="wrap"><p class="eyebrow">Shelly’s catering</p><h1>Catering that brings everyone to the table.</h1><p>From team lunches and community celebrations to traditional feasts and large public events, Shelly’s brings generous food and experienced service.</p><div class="actions">${action(business.cateringRequestUrl, 'Start a catering request')}${action(business.phoneHref, 'Call about catering', 'button--light')}</div></div></section>
      ...
      <section class="section section--cream"><div class="wrap catering-feature"><div><p class="eyebrow eyebrow--dark">What we can bring</p><h2>Comfort food for a crowd.</h2><p>Explore Three Sisters Soup with bannock or fry bread, bannock taco bars, soups and stews, fusion dishes, reception selections, barbecue menus and meals shaped around your event. Ask our team about bison stew, seasonal options and the right menu for a traditional feast.</p><div class="actions">${action(business.cateringRequestUrl, 'Browse catering options')}</div></div>${photoSlot('cateringSpread')}</div></section>
      <section class="section section--forest"><div class="wrap"><div class="section-heading section-heading--light"><div><p class="eyebrow">How a request works</p><h2>Start with the food. We’ll confirm the details.</h2></div></div><ol class="steps"><li><span>01</span><h3>Explore</h3><p>Browse the catering catalogue and choose the food that fits your gathering.</p></li><li><span>02</span><h3>Send</h3><p>Share your event details in a catering request. No payment is taken when you submit it.</p></li><li><span>03</span><h3>Confirm</h3><p>Shelly’s reviews availability, any special requests and final pricing with you.</p></li></ol><div class="actions actions--end">${action(business.cateringRequestUrl, 'Browse the catering menu')}</div></div></section>
      <section class="section section--sage"><div class="wrap split"><div><p class="eyebrow eyebrow--dark">Plan the details</p><h2>Let’s shape the meal around your event.</h2></div><div class="split__copy"><p>Ask the team about group size, service area, delivery, setup, staffing, serving style, dietary accommodation and food-allergy processes. Minimum orders, notice, deposits and cancellation terms should be confirmed before you finalize plans.</p><a class="text-link" href="${business.phoneHref}">Call ${business.phoneDisplay}</a></div></div></section>
    `
  },
  {
    path: '/our-story/',
