...
export const pages = [
  ...
  },
  {
    ...
    body: `
      ...
      <section class="section section--cream"><div class="wrap contact-grid"><div><p class="eyebrow eyebrow--dark">Where to find us</p><h2>A table in Winnipeg.</h2><p>We would love to welcome you. Please call ahead for today’s hours.</p>${photoSlot('bistroExterior')}</div><div class="contact-panel"><p class="contact-panel__label">Shelly’s Indigenous Bistro</p><address>${business.address}</address><a class="text-link" href="${business.directionsUrl}">Open directions</a><div class="contact-panel__rule"></div><p class="contact-panel__label">Phone</p><a class="contact-panel__phone" href="${business.phoneHref}">${business.phoneDisplay}</a><p class="contact-panel__label">Catering email</p><a class="contact-panel__email" href="${business.cateringEmailHref}">${business.cateringEmail}</a><p>Planning a group event? Share your details in a catering request.</p>${action(business.cateringRequestUrl, 'Start a catering request', 'button--primary')}</div></div></section>
    `
  }
];
